...
def similarity_of_set(set1, set2):
    if len(set1) != len(set2):
        set1, set2 = correct_set_size(set1, set2)

    dist = 0

    for i in range(len(set1)):
        for n in range(len(set2)):
            dist += word_similarity(set1[i], set2[n])

    return dist

def correct_set_size(set1, set2):
    if len(set1) > len(set2):
        set1 = set1[0:len(set2) - 1]
    else:
        set2 = set2[0:len(set1) - 1]
